...
=== "Fail2Ban integration"
    ...
    - Restart Fail2Ban (`systemctl restart fail2ban`, e.g.)
    - Test your settings by trying to sign in multiple times from a remote PC with a wrong user or password. After `maxretry` attempts your IP must be banned for `bantime` seconds (DietPi does not respond anymore) as the default action by Fail2Ban is `route`, specified in `/etc/fail2ban/action.d/route.conf`. You can check the current status on your DietPi with `fail2ban-client status gitea`.
    - See also:
        - [Fail2Ban](../software/system_security/#fail2ban-protects-your-system-from-brute-force-attacks)
