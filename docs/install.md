...
=== "UTM (macOS)"
    ...
    One of the options of a virtual machine is [__UTM__](https://mac.getutm.app/) for macOS.

    ![UTM Desktop DietPi machine](assets/images/UTM1.png){: width="900" height="506" loading="lazy"}

    <font size="+2">Prerequisites</font>

    As a starting point you need an **Apple Mac with a running UTM software** on which the DietPi system will run (x86 system, e.g. Mac mini 2011/2012/2014/2018 as well as M1 Apple silicon).  
    On this MAC a free harddisk space of about  
    ...
    <font size="+2">3. UTM machine for M1 (Apple silicon)</font>

    The provided UTM VM is generated for x86 systems. In case of a needed VM for Apple's M1 architecture, a new machine needs to be generated. This can be achieved by adding a new UTM VM via the `+` option in the UTM program. Then a new VM can be generated by using the DietPi images for Native PCs. Refer to the UTM documentation how to do this.  
