...
=== "UTM"
    ...
    As a starting point you can use an **Apple Mac with a running UTM software** on which the DietPi system will run (x86 system, e.g. Mac mini 2011/2012/2014/2018 as well as M1 Apple silicon).  
    On this Mac a free harddisk space of about  
    ...
    !!! info "UTM machine for M1 (Apple silicon)"
        The DietPi UTM image is an image which is x86 based. It cannot be started on a M1 Mac system, because it does not contain the emulation feature.  
        If you plan to run DietPi on a M1 Mac with UTM, it will be necessary to create an own M1 DietPi image. This can be achieved by adding a new UTM VM via the `+` option in the UTM program. Then a new VM can be generated by using the DietPi images for Native PCs. It is also an option to create an x86 DietPi image which will run on a M1 Mac. In this case you need to select the emulation option in the VM creation wizard. Refer to the UTM documentation how to do this.
        In order to have an UTM VM which also signals a VM, the procedure described in [Make your own distribution](https://dietpi.com/docs/hardware/#make-your-own-distribution) with selecting a Virtual Machine as the target needs to be executed.  
